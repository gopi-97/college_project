...
from cultivation.models import Cultivation
...
def filInventory():
    dataset = Cultivation.objects.all().filter(status='harvested')
    user = dataset.id
    product = dataset.product
    cultivated_area = dataset.cultivated_area
    FarmerInventory.objects.create(
                    user=user,
                    product=product,
                    quantity=str(int(cultivated_area.split(' ')[0])*1092) + ' kg'# Matching the cultivated area in Inventory quantity
                )
...
'''Your fillInventory() function seems to have a logical error. You're trying to access attributes like id, product, and cultivated_area directly from a queryset dataset, which will not work because dataset is a queryset, not a single object. Additionally, even if dataset contained only one object, accessing attributes directly like this would still raise an error because id, product, and cultivated_area are attributes of the Cultivation model, not the queryset.

To fix this, you need to iterate over each object in the dataset queryset and access the attributes from each object individually. Here's the corrected version of your function'''


def fillInventory():
    # Get all harvested cultivations
    harvested_cultivations = Cultivation.objects.filter(status='harvested')
    
    # Iterate over each harvested cultivation
    for cultivation in harvested_cultivations:
        # Extract necessary information
        user = cultivation.user
        product = cultivation.product
        cultivated_area = cultivation.cultivated_area

        # Create corresponding inventory entry
        FarmerInventory.objects.create(
            user=user,
            product=product,
            quantity=str(int(cultivated_area.split(' ')[0]) * 1092) + ' kg'
        )
...
def inventory_images2():
    image_folder = 'inventoryproducts/'
    inventory = FarmerInventory.objects.all()
    product_images = {
